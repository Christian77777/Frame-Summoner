...
public class Extractor
{
	...
	private boolean extractFrame(ExtractionJob job)
	{
		...
		String command = "" + escapeFilepath(prop.getProperty("FFmpeg_Path")) + " -loglevel error -y -ss " + offsetTime + " -i "
				+ escapeFilepath(videoFile.getAbsolutePath()) + " " + timecode + "-f image2 -frames:v 1 "
			...
			if (exitValue == 0)
			{
				...
				if (frameFile.exists())
				{
					...
					db.declareExtraction(completionTimestamp, job.elevated, job.author.getLongID(), job.message.getGuild().getLongID(),
							job.message.getChannel().getLongID(), job.message.getLongID(), job.video.getFilename(), offsetTime, job.frameCount,
